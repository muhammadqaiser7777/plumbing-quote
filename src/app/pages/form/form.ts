...
export class Form implements OnInit {
  ...
  validateCurrentStep(): boolean {
    ...
    if (this.currentStep === 1) {
      if (!this.PlumberType) {
        this.errors['PlumberType'] = 'Please select a service.';
        valid = false;
      }
      ...
      if (!this.BriefRequirement.trim()) {
        this.errors['BriefRequirement'] = 'Please provide a brief description of your plumbing needs.';
        valid = false;
      }
  ...
  async submit() {
    ...
    if (this.validateCurrentStep()) {
      ...
      const payload = {
        PlumberType: parseInt(this.PlumberType),
      ...
      this.http.post('https://get-plumbing.com/server/forward-lead', payload).subscribe({
        next: (response) => {
